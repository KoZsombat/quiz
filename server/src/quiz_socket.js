  ...
  io.on("connection", (socket) => {
    socket.on("joinRoom", (data) => {
      ...
      if (!roomList[rIndex].users.some(p => p.username === name)) {
        const user = { roomId: data.roomId, userId: socket.id, username: name, score: 0 };
        roomList[rIndex].users.push(user);
      } else {
        io.to(socket.id).emit("nameExist");
        return;
      }
      io.to(data.roomId).emit("usersUpdate", { count: roomList[rIndex].users.length });
    });
    ...
    socket.on("adminCon", (data) => {
      ...
      socket.join(data.quizId);
      io.to(data.quizId).emit("usersUpdate", { count: roomList[rIndex].users.length });
    ...
    socket.on("userCon", (data) => {
      ...
      if (!verified) return;
      const { name } = verified;
      const alreadyInRoom = roomList[rIndex].users.some(u => u.username === name);
      if (!alreadyInRoom) {
        const user = { roomId: data.roomId, userId: socket.id, username: name, score: 0 };
        roomList[rIndex].users.push(user);
        socket.join(data.roomId);
    ...
    socket.on("correctAns", (data) => {
      ...
      const rIndex = roomList.findIndex(r => r.roomId === data.roomId);
      if (rIndex === -1) return;
      const uIndex = roomList[rIndex].users.findIndex(u => u.username === name);
      if (uIndex !== -1) {

        ...
        <div className="w-full max-w-3xl bg-white rounded-3xl shadow-lg border border-blue-100 p-8">
          <div className="flex justify-between items-center mb-6">
            <div className="flex justify-between w-full">
              <h2 className="text-2xl font-extrabold text-blue-700 flex items-center">
                Quiz Lobby
              </h2>
              <p
                className="font-medium flex items-center text-blue-700 font-semibold cursor-pointer"
                onClick={() => {
                  if (quizId) {
                    navigator.clipboard.writeText(`${url}/broadcast/${quizId}`);
                    setAlertMsg('Broadcast link copied to clipboard!');
                  }
                }}
              >
                Boradcast Link 📋
              ...
              <p className="text-gray-500 font-medium flex items-center">
                Join Code:{' '}
                <span
                  className="text-blue-700 font-semibold cursor-pointer pl-1"
                  onClick={() => {
                    if (quizId) {
                      navigator.clipboard.writeText(quizId);
                      setAlertMsg('Join code copied to clipboard!');
                    }
                  }}
                >
                  {quizId} 📋
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-4 text-center shadow-sm">
            <p className="text-lg font-semibold text-blue-800 mb-4">
              👥 Players Joined:{' '}
              <span className="text-blue-700 font-bold">{users}</span>
            </p>

...
function App() {
  ...
  const storeUser = async (user: string) => {
    try {
      await socket.emit('setAdminUsername', { name: JSON.stringify(user) });
      socket.on('recieveAdminUsername', async (data: string) => {
    ...
    if (loginName && loginPass) {
      try {
        try {
          ...
          setAlertMsg('Error during login!');
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 flex items-center justify-center p-6">
        <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-blue-100 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-500">
            ...
            <Link
              to="/"
              className="bg-blue-700 text-white hover:bg-blue-800 px-4 py-2 h-full rounded-lg transition font-semibold"
            >
              Home
            </Link>
          </div>
